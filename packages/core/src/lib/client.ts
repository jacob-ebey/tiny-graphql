...
export interface Client {
	...
	readonly execute: <T = any>(options: ClientExecuteOptons) => Promise<ClientExecuteResult<T>>;
}
...
export function createClient(options: CreateClientOptions): Client {
	return {
		cache: options.cache,
		execute: async <T = any>(executeOptions: ClientExecuteOptons): Promise<ClientExecuteResult<T>> => {
			return ((options.cache &&
				!executeOptions.skipCache &&
				(await options.cache.read({ query: executeOptions.query, variables: executeOptions.variables }))) ||
						...
						(json) =>
							new Promise((resolve, reject) => {
								...
								if (options.cache && json) {
									...
									options.cache
										.write(
											{ query: executeOptions.query, variables: executeOptions.variables },
											json
										)
										.then(() => resolve(json))
										.catch(reject);
